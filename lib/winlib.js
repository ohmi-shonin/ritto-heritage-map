...
class WinCont {
    ...
    modal_open(p) {
        ...
        if (p.append !== undefined) {
            ...
            p.append.forEach((p) => {
                if (p.editMode == Conf.etc.editMode || p.editMode == undefined) {
                    chtml += `<button class="${p.btn_class}" data-code="${p.code}"><i class="${p.icon_class}"></i>`;
        ...
        if (p.append !== undefined) {
            p.append.forEach((buttonConfig) => {
                if (buttonConfig.editMode == Conf.etc.editMode || buttonConfig.editMode == undefined) {
                    let button = modal_footer.querySelector(`button[data-code="${buttonConfig.code}"]`);
                    if (button) {
                        button.addEventListener('click', function() {
                            if (buttonConfig.code.startsWith('javascript:')) {
                                eval(buttonConfig.code.substring(11));
                            } else {
                                eval(buttonConfig.code);
                            }
                        });
                    }
                }
...
// Force GitHub Pages cache refresh - Fri Jun 27 22:06:06 UTC 2025 - AGGRESSIVE CACHE BUST
