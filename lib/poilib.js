...
class PoiCont {
    ...
    getCatnames(tags) {		// get Category Name & subname & tag
        ...
        // building=yes を優先度を下げて処理
        let buildingKeyIndex = mainkeys.indexOf("building");
        if (buildingKeyIndex !== -1 && tags["building"] === "yes") {
            mainkeys.splice(buildingKeyIndex, 1); // building を一旦除外
            mainkeys.push("building");           // building をリストの最後に追加
        }
