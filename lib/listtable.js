...
class ListTable {
	...
	// 現在選択している時代を返す
	getSelPeriod() {
		return list_period.value
	}

	// subset of change list_period
	filterPeriod(period) {
		if (this.list.length > 0) {
			if (period === "-") {
				this.flist = this.list;
			} else {
				this.flist = this.list.filter((row) => {
					let category = row[1];
					let siteType = this.#getSiteTypeFromCategory(category);
					return this.#isPeriodMatch(siteType, period);
				});
			}
			this.grid.updateConfig({ "data": this.flist, "autoWidth": false }).forceRender(document.getElementById("tableid"));
		}
	...
	#getSiteTypeFromCategory(category) {
		if (category.includes("古墳") || category.includes("遺跡")) return "archaeological_site";
		if (category.includes("神社")) return "shrine";
		if (category.includes("寺院") || category.includes("寺")) return "temple";
		if (category.includes("教会")) return "church";
		if (category.includes("記念") || category.includes("碑")) return "monument";
		if (category.includes("宗教")) return "place_of_worship";
		return "heritage";
	}

	#isPeriodMatch(siteType, period) {
		if (!Conf.historicalPeriods || !Conf.historicalPeriods.siteMapping) return true;
		let mapping = Conf.historicalPeriods.siteMapping[siteType];
		return mapping ? mapping.includes(period) : true;
	}

	makePeriodList() {
		let oldselect = list_period.value == "" ? "-" : list_period.value;
		winCont.select_clear(`list_period`);
		winCont.select_add(`list_period`, glot.get("all_periods"), "-");
		
		if (Conf.historicalPeriods && Conf.historicalPeriods.periods) {
			Conf.historicalPeriods.periods.forEach(period => {
				winCont.select_add(`list_period`, `${period.name} (${period.range})`, period.key);
			});
		}
		
		list_period.value = oldselect;
		console.log(`listtable: makePeriodList End.`)
	}
